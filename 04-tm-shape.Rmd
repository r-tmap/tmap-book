...
The Web Mercator shown in Figure \@fig(fig:crs-05) satisfies this property; the closer an area is to one of the poles, the more it is enlarged, but since this is done in both dimensions (latitude and longitude), local shapes are preserved.
...
Figure \@fig(fig:crs-bias) shows two World maps, one in the Web Mercator projection and the other in Eckert IV projection.
Since countries with high latitude (close to the poles) are relatively large, they attract too much attention;
...
tm1 = tm_shape(World, projection = 3857) + tm_polygons("gdp_cap_est", style ="fisher", title = "GDP per capita") + tm_layout(frame = FALSE, legend.frame = TRUE, inner.margins = c(.1, .02, .02, .02), legend.position = c("left", 0.1), scale = 0.7, asp = 1)
tm2 = tm_shape(World, projection = "+proj=eck4") + tm_polygons("gdp_cap_est", style ="fisher", title = "GDP per capita") + tm_layout(frame = FALSE, legend.frame = TRUE, inner.margins = c(.1, .02, .02, .02), legend.position = c("left", 0.1), scale = 0.7, asp = 1)
