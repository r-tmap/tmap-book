...
Another example of a planar map projection is the orange shown in Figure \@ref(fig:crs-0    2).
...
There are a few factors to take into account when choosing a proper map projection.
Often, there already exists an official map projection for the area of interest, which have been chosen by a government agency.
Even though it may not always be the optimal map projection, it is nonetheless recommended to use that.
If there are no restrictions in choosing a map projection, there are two factors that are important: the application and the area of interest.



Navigation and statistical maps.



To decide wich map projection to use  s are good depends on the application and the area of interest. 

Map projections can be classified by whether the following map properties are preserved:

The standard set of comparison operators can be used in the `filter()` function, as illustrated in Table \@ref(tab:crs-properties): 
...
crs_prop_cyl = c("Mercator", "Gall-Peters, Eckert IV", "Equirectangular", "?")
crs_prop_conic = c("Lambert conformal conic", "Albers conic", "Equidistant conic", "?")
...
crs_prop_interrupted = c("Myriahedral", "Goode homolosine, Myriahedral", "none", "none")
  knitr::kable(tibble(Property = crs_prop_name, Preserves = crs_prop_attr, `Examples (cyclindrical)` = crs_prop_cyl, `Examples (conic)`=crs_prop_conic, `Examples (planar)`=crs_prop_planar, `Examples (interrupted)`=crs_prop_interrupted), 
