...
```{r crs-03, echo=FALSE, results='hide', fig.asp=0.45, warning=FALSE, fig.cap="The (interrupted) Goode homolosine projection"}
source("code/crs_examples.R")
goode = map_goode()
tm_shape(goode$bg) +
...
These are depicted in Figure \@(fig:crs-types).
...
```{r crs-eqdc, echo=FALSE, results='hide', warning=FALSE, fig.cap="World Equidistant Conic", message=FALSE}
source("code/crs_examples.R")
meqdc = map_eqdc()
tm_shape(meqdc$bg) +
    tm_polygons(clr_bg, border.col = clr_border) +
...
crs_prop_name = c("Conformal", "Equal area", "Non-interrupted", "")
crs_prop_subname = c("", "", "\\- Equidistant", "\\- Azimuthal")
crs_prop_attr = c("Local angle (shape)", "Area", "Distance", "Direction")
knitr::kable(tibble(Property = crs_prop_name, ` ` = crs_prop_subname, `Preserves` = crs_prop_attr), 
