...
The **tmap** package has many defaults that automatically specifies colors, sizes, and shapes used on the maps. 
...
For example, for colors, it can be a vector of colors, a palette function, or a name of a build-in palette; for sizes, it can be a numerical variable; and for shapes, it can be a custom icon or a numerical variable.
<!-- ref to the figure below -->
...
tm_shape(volcanos) +
  tm_symbols(col = "elevation",
             col.scale = tm_scale(values = "greens"))
tm_shape(volcanos) +
