...
#| message: false
#| fig-asp: 0.4
#| fig-cap: 'Basic representation of the raster data model: (1) Cell IDs, (2) Cell values,
#|   and (3) A raster map'
...
tm_raster2 = tm_shape(sta_sf) +
  tm_polygons("A1.1") +
  tm_text("A1.2") +
  tm_title("2. Cell values") +
  tm_layout(frame = FALSE, legend.show = FALSE)

tm_raster3 = tm_shape(sta) +
  tm_raster("A1.1") +
  tm_title("3. Raster map") +
...
#| warning: false
#| fig-asp: 0.4
#| fig-cap: 'Main types of raster data grids: (1) Regular, (2) Rotated, (3) Sheared,
#|   (4) Rectilinear, and (5) Curvilinear'
# regular grid 
...
tmrgb1 = tm_shape(landsat234[[3]]) +
  tm_raster(col.scale = tm_scale(values = "Greys")) + 
  tm_title("Red") +
  tm_layout(frame = FALSE, legend.show = FALSE)
tmrgb2 = tm_shape(landsat234[[2]]) +
  tm_raster(col.scale = tm_scale(values = "Greys")) + 
  tm_title("Green") +
  tm_layout(frame = FALSE, legend.show = FALSE)
tmrgb3 = tm_shape(landsat234[[1]]) +
  tm_raster(col.scale = tm_scale(values = "Greys")) + 
  tm_title("Blue") +
  tm_layout(frame = FALSE, legend.show = FALSE)
...
#| warning: false
#| message: false
#| fig.cap: The WGS84 coordinate system (EPSG4326)
#| eval: false
...
#| message: false
#| eval: false
#| fig-cap: Web Mercator projection (EPSG3857)
source("code/crs_examples.R")
...
An example of a conic map projection is shown in @fig-crs-conic-planar(a).
...
An example is the Lambert Azimuthal Equal-Area projection (EPSG3035), shown in @fig-crs-conic-planar(b), which is optimized for Europe.
...
#| warning: false
#| message: false
#| fig-cap: Examples of a conic (a) and a planar (b) projection.
source("code/crs_examples.R")
meqdc = map_eqdc()
m3035 = map_3035()
...
tm_shape(meqdc$grat) +
    tm_lines(col = clr_grat, lwd = 1) +
    tm_layout(frame = FALSE) +
    tm_title("(A) World Equidistant Conic projection", size = 0.9, color = "#777777", position = c("center", "bottom")) +
...
tm_shape(m3035$grat) +
    tm_lines(col = clr_grat, lwd = 1) +
    tm_layout(frame = FALSE) +
    tm_title("(B) Lambert Azimuthal Equal-Area projection", size = 0.9, color = "#777777", position = c("center", "bottom")) +
    tm_layout(inner.margins = c(.1, 0.02, 0.02, 0.02))
})
```
...
For instance, a standard for Europe, which is used by Eurostat (the statistical agency of the European Union), is the Lambert Azimuthal Equal-Area projection shown in @fig-crs-conic-planar (b).
...
tm1 = tm_shape(World, crs = "EPSG:3857") + 
  ...
  tm_layout(frame = FALSE, legend.frame = TRUE, 
            ...
            scale = 0.7, asp = 1)
tm2 = tm_shape(World, crs = "+proj=eck4") + 
  tm_polygons("pop_est_dens",
              fill.scale = tm_scale_continuous_log10(),
              fill.legend = tm_legend(expression("Population per " * km^2), 
                                      orientation = "landscape")) +
...
For World maps, pseudo-cylindrical map projections, such as the Robinson projection (@fig-crs-robin) and the Eckert IV projection (@fig-crs-bias(b)) are very popular because they have less distortion other map projections. 
...
However, other hemispheres are often used implicitly, such as a hemisphere centered on Europe used in the Lambert Azimuthal Equal-Area projection shown in @fig-crs-conic-planar(b).
...
#| echo: false
#| warning: false
#| fig-cap: 'Comparison between the same dataset of three waterfalls using: (A) the WGS84
#|   coordinate system, (B) the Equal Earth projection.'
ctp1 = tm_shape(World, crs = "EPSG:4326") +
    tm_polygons() +
  tm_shape(waterfalls_sf) +
    tm_markers(text = "name") +
  tm_grid() +
  tm_title_in("A")
ctp2 = tm_shape(World, crs = "EPSG:8857") +
    tm_polygons() +
  tm_shape(waterfalls_sf_trans) +
    tm_markers(text = "name") +
  tm_grid() +
  tm_title_in("B")
tmap_arrange(ctp1, ctp2, ncol = 1)
```
