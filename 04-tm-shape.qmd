...
#| fig-asp: 0.35
#| fig-cap: A map representing three shapes (worldelevation, worldvector, and worldcities)
#|   using four layers and zoomed into the locations in the worldcities object.
tm_shape(worldelevation) +
  tm_raster("worldelevation.tif", 
            col.scale = tm_scale(values = terrain.colors(8))) +
tm_shape(worldvector) +
...
@fig-tm-map-proj A shows the world elevation raster reprojected to Equal Earth.
...
@fig-tm-map-proj B shows an example of the second approach, which gave a better result in this case without any spurious lands.
However, creation of the B map takes about ten times longer than the A map.

```{r}
#| warning: false
#| eval: false
tm_shape(worldelevation, crs = "EPSG:8857", raster.warp = FALSE) +
  tm_raster("worldelevation.tif", 
            col.scale = tm_scale(values = terrain.colors(8)))
```
...
#| echo: false
#| warning: false
#| fig-cap: 'Two elevation maps in the Equal Earth projection: (A) created using raster.warp
#|   = TRUE, (B) created using raster.warp = FALSE.'
...
@fig-vectordown A shows a map of countries from the `worldvector` object.
...
In the example below, we set `simplify` to 0.05, which keeps 5% of vertices (@fig-vectordown B).
...
@fig-vectordown C shows the result of such an operation.
...
@fig-vectordown D contains a simplified map, where each spatial geometry of the original map still exists, but in a less detailed form.

```{r}
#| label: fig-vectordown
#| echo: false
#| message: false
#| fig-cap: "A map of world's countries based on: (A) original data, (B) simplified data with 5% of vertices kept, (C) simplified data with 5% of vertices, and all features kept, (D) simplified data with 5% of vertices, all features, and all polygons kept."
...
Raster downsampling can be also disabled with the `raster.downsample` argument of `tm_shape()` (@fig-rasterdown B).
...
#| echo: false
#| message: false
#| fig-cap: (A) A raster map with the decreased resolution, (B) a raster map in the original
#|   resolution.
tmap_options(raster.max_cells = c(plot = 5000, view = 2000))
