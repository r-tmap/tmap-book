...
`tm_polygons()` accepts three ways of specifying the fill color with the `fill` argument.
...
<!-- what is the default tm_scale_ here? -->
...
The `tm_polygons(fill = "region_un", fill.scale = tm_scale_categorical())` code is run automatically in this case.
...
<!--should we explain how to do it? -->
As mentioned in the @sec-color-palettes, we can also change the used color palette with the `values` argument.
...
**tmap** has several different methods to create discrete maps<!--list??--> that can be specified with by the one of the functions from the `tm_scale_` family.
...
By default, the `"tm_scale_discrete"` function is used in the background (@fig-discrete-methods-1).
...
The next approach is to manually select the limits of each break with the `breaks` argument of `tm_scale()` (@fig-discrete-methods-2).
...
The most common optimization method used in cartography is the Jenks optimization method implemented at the `"jenks"` style (@fig-discrete-methods-3).

<!-- how about adding ggplot2 histograms?? -->
<!-- should we add that these methods usually do not allow to compare between datasets? -->

```{r}
#| label: discrete-methods3
#| eval: false
tm_shape(worldvector) +
  tm_polygons(fill = "gdp_per_cap", 
...
The `"log10_pretty"` style uses a logarithmic base-10 transformation (@fig-discrete-methods-4).
...
#| echo: false
#| layout-ncol: 2
#| fig-cap: Examples of four methods of creating discrete maps
#| fig-subcap:
#|   - The "pretty" method
#|   - The "fixed" method
#|   - The "jenks" method
...
Finally, the `tm_scale_continuous_log10` scale is the continuous equivalent of the `"log10_pretty"` style (@fig-cont-methods-3).
