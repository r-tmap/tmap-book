...
tm_shape(meqdc$grat) +
    tm_lines(col = clr_grat, lwd = 1) +
    tm_layout(frame = FALSE) +
    tm_title("(A) World Equidistant Conic projection", size = 0.9, 
             color = "#777777", position = c("center", "bottom")) +
...
tm_shape(m3035$grat) +
    tm_lines(col = clr_grat, lwd = 1) +
    tm_layout(frame = FALSE) +
    tm_title("(B) Lambert Azimuthal Equal-Area projection", size = 0.9,
             color = "#777777", position = c("center", "bottom")) +
    tm_layout(inner.margins = c(.1, 0.02, 0.02, 0.02))
})
```
...
tb = tibble(Property = crs_prop_name, Preserves = crs_prop_attr, 
