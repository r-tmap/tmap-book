...
attr(sta_rectilinear, "dimensions")[[2]]$delta = -1
sta_rectilinear_sf = st_as_sf(sta_rectilinear)
tm_shape(sta_rectilinear_sf) +
  tm_polygons() +
  tm_text("m") +
  tm_layout(frame = FALSE)
# curvilinear grids
# sta_curvilinear = sta
# X1 = matrix(rep(c(1, 2, 3, 4, 5), 4), nrow = 4, ncol = 5, byrow = TRUE)
# X1 = matrix(c(1, 1, 2, 2, 3, 1, 1, 2, 2, 3, 3, 3, 4, 4, 5, 5, 4, 4, 5, 5), nrow = 4, ncol = 5)
# X2 = matrix(rep(c(1, 2, 3, 4), 5), nrow = 4, ncol = 5)
# sta_curvilinear = st_as_stars(sta_curvilinear, 
#                               curvilinear = list(x = X1,
#                                                  y = X2))
# plot(sta_curvilinear)
# sta_curvilinear_sf = st_as_sf(sta_curvilinear)
# tm_shape(sta_curvilinear_sf) +
#   tm_polygons(col = "A1", palette = "viridis") +
#   tm_text("A1") + 
...
temp <- read.table(f)#[1:20, ]
x = matrix(temp$V1,nrow=265, ncol=166)
y = matrix(temp$V2,nrow=265, ncol=166)
am = matrix(temp$V3, nrow=265, ncol=166)
s = st_as_stars(am)
plot(s)
