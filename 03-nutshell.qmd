...
Additionally, **tmap** makes it possible to create small multiples map (@sec-sm-section) and map animations (@sec-ani-section).
...
@fig-qtm A shows a visualization of the geometries from the `volcanoes`.
...
For example, we can change the shapes of the points in `volcanoes`, make their sizes related to the the `"elevation"` argument, and add a title (@fig-qtm B).
...
#| label: fig-qtm
#| echo: false
#| fig-cap: Example of a quick map created with the `qtm()` function. A) A map with geometries only. B) A map with geometries and attributes.
tmq1 = qtm(volcanoes) + tm_title("A")
tmq2 = qtm(volcanoes, shape = 24, size = "elevation", title = "Volcanoes") +
   tm_title("BVolcanoes")
...
tm_shape(ei_elev) +
  ...
  tm_borders() +
  tm_shape(ei_roads) +
  tm_lines(lwd = "strokelwd", lwd.legend = tm_legend_hide()) +
  tm_shape(volcanoes) +
  tm_symbols(shape = 24, size = "elevation",
...
my_map = tm_shape(ei_elev) +
  ...
  # tm_add_legend(type = "lines", col = "black", title = "Roads") +
  tm_title("Easter Island") +
  tm_layout(bg.color = "lightblue")
```
...
<!-- improve -->
It shows our spatial data using similar aesthetics to @fig-rmap2, but allows us to zoom in and out or move the map.
