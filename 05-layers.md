...
Two basic methods are `"equal"` and `"quantile"` styles.
...
The `"quantile"` style, on the other hand, creates several classes with exactly the same number of objects (e.g., spatial features), but having intervals of various sizes.
This method has an advantage or not having any empty classes or classes with too few or too many values.

However, the resulting intervals from the `"quantile"` style can often be misleading, where very different values could be located in the same class.

<!-- EXT: A quantile classification is well suited to linearly distributed data.  -->
