...
```{r tmpolygons, warning=FALSE, fig.cap="Example of: (A) a default map created with 'tm_polygons()', (B) a map created with user-provided colors.", echo=FALSE}
tmp1 = tm_shape(x) +
  tm_polygons() +
  tm_layout(title = "A")
tmp2 = tm_shape(x) +
...
```{r tmpolygonsder, warning=FALSE, fig.cap="Example of: (A) a map created with 'tm_fill()', (B) a map created with 'tm_borders()'.", echo=FALSE}
tmpa1 = tm_shape(x) +
  tm_fill() +
  tm_layout(title = "A")
tmpa2 = tm_shape(x) +
  tm_borders() +
  tm_layout(title = "B")
tmap_arrange(tmpa1, tmpa2, ncol = 1)
```
