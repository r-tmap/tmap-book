...
```{r colorscales1, warning=FALSE, fig.cap="Example of a map with all polygons filled with the same color (left) and with adjacent polygons having different colors (right).", echo=FALSE, fig.asp=NA, fig.height=2}
tm_one = tm_shape(x) +
  tm_polygons(col = "lightblue")
tm_uni = tm_shape(x) +
  tm_polygons(col = "MAP_COLORS")
tmap_arrange(tm_one, tm_uni, nrow = 1, asp = NA,
             outer.margins = 0)
```
...
<!-- categorical -->
```{r, warning=FALSE, fig.asp=NA, fig.height=2, message=FALSE, echo=FALSE}
tm_cat = tm_shape(x) +
  tm_polygons(col = "region_un")
tm_cat
```
