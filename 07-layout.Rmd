...
Information about installing and debugging custom fonts can be found in [a blog post](https://yjunechoe.github.io/posts/2021-06-24-setting-up-and-debugging-custom-fonts/) by June Choe and in the **showtext** package [@R-showtext] documentation.

The next step is to either view or save our map.
...
In case of any problems with graphical raster outputs, it is possible to try graphics devices implemented in the **ragg** package [@R-ragg].
...
When the PDF reader needs to show us the document, then it tries to locate the font on your computer, and can use other fonts when the expected one does not exist.
An alternative approach is called embedding, which adds a copy of each necessary font to the PDF file itself.
Gladly a few possible technical solutions to create PDFs with proper fonts exist.
Firstly, it could be worth trying some alternative graphic devices such as `cairo_pdf` or `svglite::svglite`.
The second option is to try the **showtext** package [@R-showtext], which converts text into color-filled polygonal outlines for graphical vector formats.
