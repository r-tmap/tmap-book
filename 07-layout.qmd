...
#| label: fig-fonts
#| echo: false
#| fig-cap: Basic (A) font families, and (B) font faces implemented in the tmap package.
tmap_arrange(tm_ff + tm_title("A. Font families"), tm_bf + tm_title("B. Font faces"), nrow = 1)
```

In **tmap**, fonts are represented by a font family (@fig-fonts A) and a font face (@fig-fonts B).
...
Notice, for example, short horizontal lines on the bottom of letters *r*, *i*, and *f* or vertical lines at the ends of the letter *s* in the top row of @fig-fonts A.
...
For example, `serif` could the `Times` font family, `sans` - `Helvetica` or `Arial`, and `monospace` - `Courier` (@fig-fonts A).
The second mechanism allows to select a font family based on its name (e.g., `Times` or `Palatino`).
Next, a member of the selected font families can be selected with one of the font faces: `plain`, `italic`, `bold`, and `bold.italic` (@fig-fonts B).
...
The first one is to change all of the fonts and font faces for the whole map at once (@fig-mfonts A).
...
The second way is to specify just some text elements independently (@fig-mfonts B).
...
#| label: fig-mfonts
#| echo: false
#| fig-cap: Examples of (A) one font (font family and font face) used for all of the
#|   map elements (title, text labels, legend, and text annotation), and (B) different
...
For example, the `ei_borders` object's CRS is UTM zone 12S with the units in meters (@fig-grids A).
...
This can be seen with the degree signs in the labels (@fig-grids B).
...
On the other hand, when `tm_grid()` or `tm_graticules()` is placed before the map layer code, the grid lines are plotted behind the spatial data (@fig-grids C).
...
#| label: fig-grids
#| echo: false
#| fig-cap: (A) Map grid, (B) graticules, and (C) graticules put behind the map layer.
#| fig-asp: 0.33
tmap_arrange(
    <<grid1>>
      + tm_title("A"),
...
## Layout elements {#sec-layout-elements}
