...
In the next example, we put the vector `"CartoDB.VoyagerNoLabelsHybrid"` tiles on top of the previously set basemaps, but below the dots layer (Figure \@ref(fig:tmtiles1)).
...
The code below plots the `"CartoDB.VoyagerNoLabels"` tiles in the background, adds the island outline in light blue color, and puts attribution text in the bottom right corner of the map (Figure \@ref(fig:stiles)).
...
(ref:stiles) Example of a static map using a downloaded `"CartoDB.VoyagerNoLabels"` tile layer. 
