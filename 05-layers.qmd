...
In the next example, we put the vector `"CartoDB.VoyagerNoLabelsHybrid"` tiles on top of the previously set basemaps, but below the dots layer (@fig-tmtiles1).

```{r}
#| eval: false
tm_basemap(c(StreetMap = "OpenStreetMap",
             TopoMap = "OpenTopoMap")) +
  tm_tiles(c(TonerHybrid = "CartoDB.VoyagerNoLabelsHybrid")) +
  tm_shape(volcanos, is.master = TRUE) + 
  tm_dots(col = "red", group = "Volcanos")
```
...
#| eval: false
library(maptiles)
ei_tiles = get_tiles(ei_borders, provider = "CartoDB.VoyagerNoLabels", zoom = 12, crop = TRUE)
```
